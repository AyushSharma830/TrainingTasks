...
class StudentValidation @Inject constructor(private val studentRepository: StudentRepository) {
    ...
    fun validateRollNo(rollNo : String?){
        require(!rollNo.isNullOrBlank()) { "Student RollNo. can not be Null or Blank." }
    }

    fun validateEmailUniqueness(email : String, existingStudentId: String?){
        require(studentRepository.validateEmailUniqueness(email, existingStudentId)) { "Student Email should be unique." }
    }

    fun validateRollNoUniqueness(rollNo : String, existingStudentId: String?, schoolId : String?){
